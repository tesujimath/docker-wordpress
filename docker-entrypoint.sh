...
if test -f /var/www/html/${WORDPRESS_CORE}/wp-activate.php; then
    echo >&2 "WordPress already downloaded"
else
    cd /var/www/html
    curl -o wordpress.tar.gz -fSL https://wordpress.org/wordpress-${WORDPRESS_VERSION}.tar.gz
    echo "${WORDPRESS_SHA1} *wordpress.tar.gz" | sha1sum -c -
    rm -rf ${WORDPRESS_CORE}
    mkdir -p ${WORDPRESS_CORE}
    # upstream tarballs include ./wordpress/ which we strip
    tar -xzf wordpress.tar.gz --strip-components=1 -C ${WORDPRESS_CORE}
...
su -s /bin/sh -c "wp --path=/var/www/html/${WORDPRESS_CORE} core is-installed || wp --path=/var/www/html/${WORDPRESS_CORE} core install --url=\"${WORDPRESS_URL}\" --title=\"${WORDPRESS_TITLE}\" --admin_user=\"${WORDPRESS_ADMIN_USER}\" --admin_password=\"${WORDPRESS_ADMIN_PASSWORD}\" --admin_email=\"${WORDPRESS_ADMIN_EMAIL}\"" www-data
